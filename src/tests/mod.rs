...
mod tests {
    ...
    pub fn test_contribute_instruction() {
        ...
        let mut contributor_account = svm
            .get_account(&contibutor_PDA.0)
            .expect("Vault ATA should exist");

        // let contributor_account_state =
        //     crate::state::ContributorData::try_deserialize(contributor_account.data.as_ref())
        //         .unwrap();
        let mut pda_amount: u64 = 0;
        let amt_bytes = contributor_account.data.clone();
        let amt_arr: [u8; 8] = amt_bytes.try_into().unwrap();
        pda_amount = u64::from_le_bytes(amt_arr);

        assert_eq!(pda_amount, amount_to_contribute);

        // let contributor_account_state = load_acc_data_mut_unchecked::<ContributorData>(
        //     contributor_account.to_account_shared_data().data_mut(),
        // )
        // .unwrap();
    }
